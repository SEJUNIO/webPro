	...
	<script>
		$(document).ready(function() {
			$('tr').click(function(){
				...
				if(!isNaN(Number(mid))){
					location.href = '${conPath }/content.do?bid=${dto.bid}&pageNum=${pageNum}&bid='+bid;
				}
			});
		});
	</script>
...
<body>
	...
	<c:if test="${modifyResult eq SUCCESS }">
		<script>alert('${param.bid}번글수정 성공');</script>
	</c:if>
	<c:if test="${modifyResult eq FAIL }">
		<script>alert('${param.bid}번글수정 실패'); history.back();</script>
	</c:if>
	...
	<table>
		...
		<%-- <c:if test="${list.isEmpty()"} 아래 size와 같다 --%>
