...
<body>
	<h1>특정 세션 값 가져오기</h1>
	<% //특정 세션 값 가져오기(세션의 특정한 속성갑 가져오기)
		String sessionName = (String)session.getAttribute("sessionName"); //오브젝트 타입이라 String으로 형변환
		Integer sessionNum = (Integer)session.getAttribute("sessionNum");
		Member member = (Member)session.getAttribute("member"); //Member로 형변환
		String noSessionAttr = (String)session.getAttribute("noSessionAttr"); //null값
		...
		if(member!=null){
			...
			out.println("<h2>세션에 member속성이 없습니다</h2>");
		}
	%>
